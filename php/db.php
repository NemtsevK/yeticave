...
{
    $sql = "SELECT * FROM yeticave_users WHERE email = ?";
...
{
    $sql = "SELECT name FROM yeticave_users WHERE id = ?";
...
{
    $categories_list = "SELECT * FROM yeticave_categories";
...
{
    $sql = "SELECT contact FROM yeticave_users WHERE id = $id";
...
{
    $sql = "SELECT name FROM yeticave_categories WHERE id = $id";
    $result = mysqli_query($link, $sql);
    if (!$result) {
        dbError($link);
    }
 ...
 * @var int $lots_per_page Количество лотов на странице
 */

function getLots(mysqli $link, int $lots_per_page, int $current_page_number): array
{
...
function addUser(mysqli $link, array $form_data): void
{
    $sql = "INSERT INTO yeticave_users (name, email, password, contact)
            VALUES (?, ?, ?, ?)";
    $data = [
        $form_data['name'],
        $form_data['email'],
...
{
    $sql = "SELECT id AS lotId, title AS lotTitle, user_id AS lotAuthor FROM yeticave_lots l
...
{
    $sql = "SELECT COUNT(*) FROM yeticave_lots WHERE category_id = $id";
    $result = mysqli_query($link, $sql);
    if (!$result) {
        dbError($link);
    }
...
{
    $sql = "INSERT INTO yeticave_bets (date_create, price, user_id, lot_id) VALUES (?, ?, ?, ?)";
    $data = [
        date('Y-m-d H:i:s'),
        $price,
        $user,
...
function getAllMyBets(mysqli $link, int $id): ?array
{
    $sql = "SELECT b.price, b.date_create, l.title, l.image, l.id AS lot_id, l.finish_date, c.name AS category_name, l.winner_id, l.title
            FROM yeticave_bets b
                     JOIN yeticave_lots l ON l.id = b.lot_id
                     JOIN yeticave_categories c ON c.id = l.category_id
            WHERE b.user_id = $id
            ORDER BY l.finish_date DESC;";
    $result = mysqli_query($link, $sql);
    if (!$result) {
...
{
    $sql = "SELECT * FROM yeticave_bets WHERE lot_id=? ORDER BY (date_create) DESC LIMIT 1";
    $data = [$lot];
    $stmt = db_get_prepare_stmt($link, $sql, $data);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
...
{
    $sql = "SELECT * FROM yeticave_users WHERE id = $id";
    $result = mysqli_query($link, $sql);
    return mysqli_fetch_assoc($result);
}
...
{
    $sql = "UPDATE yeticave_lots SET winner_id = $user_id WHERE id = $lot_id";
    $result = mysqli_query($link, $sql);
    if (!$result) {
        dbError($link);
    }
 ...
 * @param int $current_page_number Номер текущей страницы
 * @return array Возвращает список лотов
 */

function searchLots(mysqli $link, string $search, int $lots_per_page, int $current_page_number): array
{
    $offset = ($current_page_number - 1) * $lots_per_page;
...
{
    $sql = "SELECT COUNT(*) FROM yeticave_lots WHERE finish_date > NOW()";
    $result = mysqli_query($link, $sql);
    if (!$result) {
        dbError($link);
    }
...
function getLotsByCategory(mysqli $link, int $category, int $lots_per_page, int $current_page_number): array
{
    $offset = ($current_page_number - 1) * $lots_per_page;
    ...
    $data = [$category, $lots_per_page, $offset];
    $stmt = db_get_prepare_stmt($link, $sql, $data);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
...
{
    $sql = "SELECT * FROM yeticave_bets WHERE lot_id=? ORDER BY (date_create) DESC ";
    $data = [$lot];
    $stmt = db_get_prepare_stmt($link, $sql, $data);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
...
{
    $sql = "INSERT INTO yeticave_lots (date_create, title, description, image, price, finish_date, step, user_id, category_id)
            VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?)";
